...
def paste_photo(card, photo_path):
    ...
    draw.rounded_rectangle(PHOTO_BOX, radius=PHOTO_RADIUS, fill=BLACK, outline=GOLD, width=4)

    if not photo_path.exists():
        draw.rounded_rectangle((left+8, top+8, right-8, bottom-8), radius=PHOTO_RADIUS-6, fill=(16, 16, 16))
        f = get_font(FONT_BOLD, 32)
        msg = "FOTO\nMANCANTE"
        bbox = draw.multiline_textbbox((0, 0), msg, font=f, spacing=8, align="center")
        x = left + (width - (bbox[2] - bbox[0])) / 2
        y = top + (height - (bbox[3] - bbox[1])) / 2
        draw.multiline_text((x, y), msg, font=f, fill=(180, 180, 180), spacing=8, align="center")
        return

    img = Image.open(photo_path).convert("RGB")
    img = ImageOps.fit(img, (width-12, height-12), method=Image.Resampling.LANCZOS, centering=(0.5, 0.35))
    mask = rounded_mask(img.size, PHOTO_RADIUS-8)
    card.paste(img, (left+6, top+6), mask)
    draw.rounded_rectangle(PHOTO_BOX, radius=PHOTO_RADIUS, outline=GOLD, width=4)
...
def draw_member_data(card, row):
    ...
    number_font = get_font(FONT_BOLD, 38)
    date_font = get_font(FONT_BOLD, 38)
    validity_font = get_font(FONT_BOLD, 35)

    draw.text(NAME_POS, full_name, font=name_font, fill=WHITE, stroke_width=1, stroke_fill=(80, 80, 80))
    draw.text(ROLE_POS, qualifica, font=role_font, fill=GOLD, stroke_width=1, stroke_fill=(45, 35, 0))
    draw.text(CARD_NO_POS, numero, font=number_font, fill=WHITE)
    if data:
...
def render_one(template, row, photos_dir, output_dir):
    ...
    paste_photo(card, photos_dir / foto if foto else Path("__missing__"))
...
def main():
    parser = argparse.ArgumentParser(description="Genera tessere Club Napoli Romazzurra da CSV.")
    ...
    template = Image.open(BASE_DIR / args.template).convert("RGB")
    if template.size != CANVAS_SIZE:
        raise ValueError(f"Template atteso {CANVAS_SIZE}, trovato {template.size}. Aggiorna config.py.")

    df = pd.read_csv(BASE_DIR / args.csv, dtype=str).fillna("")
    required = {"nome", "cognome", "numero_tessera", "qualifica", "foto"}
    missing = required - set(df.columns)
    if missing:
        raise ValueError("Colonne mancanti nel CSV: " + ", ".join(sorted(missing)))

    output_dir = BASE_DIR / args.output
    output_dir.mkdir(exist_ok=True)
    photos_dir = BASE_DIR / args.photos

    generated = [render_one(template, row, photos_dir, output_dir) for _, row in df.iterrows()]
    if args.pdf:
